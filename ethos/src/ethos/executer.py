...
def create_enviroment():
    ...
    for i in list_traits:
        ...
        if not manifest_file.exists():
            print(f"Warning: Trait {i} does not have a manifest.json file. Failed to load it. Skipping....")
            continue
        with open(manifest_file, "r") as f:
            try:
                manifest = json.load(f)
            except:
                print(f"Warning: Trait {i} manifest.json file is invalid and does not contains proper content, skipping it....")
                continue
        binary_path=manifest.get("binary")
        if not (hard_traits_path/i/binary_path).exists():
            print(f"Warning: Trait {i} binary path defined in manifest.json is invalid, no file is found at that path. Failed to load it.Skipping....")
            continue
        binary=ctypes.CDLL(str(hard_traits_path / i / binary_path))
        if not "functions" in manifest:
            print(f"Warning: Trait {i} manifest.json does not have functions key defined,failed to load trait {i}. Skipping it....")
            continue
        for j in manifest["functions"]:
            ...
            try:
                ...
                print(f"Warning: In trait {i} there is a function named {j} of which types are not correctly written in manifest.json, skipping loading it....")
                continue
